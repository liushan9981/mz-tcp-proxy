...
func printGoroutineInfo() {
	for {
		...
		for i := 0; i < WORKERCOUNT; i++ {
			...
			worker_info.mutex_ConnectionInfoStatus.RLock()
			fmt.Println("worker_info.connCliUpstreamInfo:", worker_info.ConnectionInfoStatus)
			worker_info.mutex_ConnectionInfoStatus.RUnlock()
